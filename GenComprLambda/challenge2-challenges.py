...
to_forest = [(loc, locations[loc]) for loc in locations if 5 in exits[loc].values()]
print(to_forest)

for location in locations:
    to_loc = [(loc, locations[loc]) for loc in locations if location in exits[loc].values()]
    print("{}: {}".format(location, to_loc))

print('*' * 40)

to_forest = []
for loc in locations:
    if 5 in exits[loc].values():
        to_forest.append((loc, locations[loc]))
print(to_forest)

for location in locations:
    to_loc = []
    for loc in locations:
        if location in exits[loc].values():
            to_loc.append((loc, locations[loc]))
    print("{}: {}".format(location, to_loc))
